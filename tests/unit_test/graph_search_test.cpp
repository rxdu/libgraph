...
struct SimpleState {
  SimpleState(int64_t row, int64_t col) : row_(row), col_(col) {}
  ...
  int64_t GetUniqueID() const {
    // You can return the state id directly if you have one and it's unique (see
    // StateExample class) or you can use some kind of hash functions to
    // generate one
...
TEST_F(GraphSearchTest, ValueTypeDijkstra) {
  ...
  ASSERT_TRUE(path_ids == spath || path_ids == spath2)
      << "Path found by Dijkstra in value-type graph is not correct";
}
...
TEST_F(GraphSearchTest, PointerTypeDijkstra) {
  ...
  ASSERT_TRUE(path_ids == spath || path_ids == spath2)
      << "Path found by Dijkstra in pointer-type graph is not correct";
}
...
TEST_F(GraphSearchTest, ValueTypeAStar) {
  ...
  ASSERT_TRUE(path_ids == spath || path_ids == spath2)
      << "Path found by A* in value-type graph is not correct";
}
