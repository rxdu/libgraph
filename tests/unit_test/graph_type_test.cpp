...
struct SharedPtrTestStateIndexer {
  int64_t operator()(std::shared_ptr<TestState> state) { return state->id_; }
};
...
TEST_F(GraphTypeTest, ValueType) {
  ...
  ASSERT_EQ(graph.GetTotalVertexNumber(), 9)
      << "Failed to add vertices to value-type graph ";
  ASSERT_EQ(graph.GetTotalEdgeNumber(), 20)
      << "Failed to add vertices to value-type graph ";
}
